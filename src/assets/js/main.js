...
$(function() {
    ...
    $(".header-main .navbar-main .dropdown").on("shown.bs.dropdown", function() {
        toggleHeader($(this));
    });
    // Toggle navigation when dropdown hidden
    $(".header-main .navbar-main .dropdown").on("hidden.bs.dropdown", function() {
        toggleHeader($(this));
    });
    ...
    $(".scrollspy-leftnav ul a").click(function() {
        ...
        $("body").animate(
            {scrollTop: target_top}, 1000, function() {
    ...
    function firstDropdownItem(target) {
        const nav_tab_active = $(`${target} .dropdown-item`).first().addClass("d-none").html();
        $(`${target} [data-bs-toggle="dropdown"] span`).html(nav_tab_active);
    }

    function dropdownClick(target) {
        const current = target.html().split("(")[0];
        const btn_target = target.parents('.dropdown').find('[data-bs-toggle="dropdown"]').find("span");
    ...
    $('.vertical-tabs .dropdown-item, .horizontal-tabs .dropdown-item, .glossary-index__nav .dropdown-item, .side-nav-container .dropdown-item').click(function() {
        dropdownClick($(this));
    })
    ...
    function glossaryScroll(t, type) {
        ...
        if (type === 'desktop') {
            ...
            {scrollTop: target_top}, 500, function() {
                $("body").removeClass("nav-hidden").addClass("nav-hide");
            }
        );
        }
    ...
    function resizeBannerBox(type) {
        ...
        const box_height = $('.banner--box .banner__content').outerHeight()/divider;
    ...
    $(window).resize(function() {
        (window_size >= 576) ? resizeBannerBox('desktop') : resizeBannerBox('mobile');
    })
    ...
    $(".featured-blog-carousel .carousel-main").slick({
        ...
        slidesToShow: 1,
        slidesToScroll: 1,
        prevArrow: "<img class='prev slick-prev' src='assets/img/chevron-circle-blue-left.svg'>",
        nextArrow: "<img class='next slick-next' src='assets/img/chevron-circle-blue-right.svg'>",
        responsive: [
            {
                breakpoint: 576,
                settings: {
    ...
    $(".features-carousel .carousel-main").slick({
        ...
        slidesToShow: 1,
        slidesToScroll: 1,
        prevArrow: "<img class='prev slick-prev' src='assets/img/chevron-circle-white-left.svg'>",
        nextArrow: "<img class='next slick-next' src='assets/img/chevron-circle-white-right.svg'>",
        responsive: [
            {
                breakpoint: 576,
                settings: {
    ...
    $('.modal').on('shown.bs.modal', function (e) {
        $('.quotes-img-carousel, .upcoming-events-carousel').slick('refresh').css('opacity', 1);
    });
    ...
    $(window).on('activate.bs.scrollspy', function(e, obj) {
        ...
        if (item.parents("li").is(":first-child") || item.parents("li").is(":last-child")) {
    ...
    setInterval(() => {
        ...
        const target = $(`.good-company .row-main > [class*="col-"]:nth-child(${current_gc + 1})`);
        const max = target.find('picture').length;
        const current_active = target.find('picture.active');
        const next_active = (current_active.index() === max - 1) ? target.find('picture').first() : target.find('picture.active').next();

        current_active.removeClass('active');
        next_active.addClass('active');
        
        current_gc = (random === current_gc) ? Math.floor(Math.random() * max_gc) : random;
